...
class AVQA_dataset(Dataset):
    def __init__(self, label, audio_dir, video_dir, question_dir,
        ...
        self.dir_fps_path = r'E:\DataSet\MUSIC-QA\frames-1fps'
    ...
    def __getitem__(self, idx):
        ...
        sample_indx = np.linspace(0, 58, num=20, dtype=int)
        video_img_list = sorted(glob.glob(os.path.join(self.dir_fps_path, name, '*.jpg')))
        print(len(video_img_list))
        print(sample_indx)
        img_list = [video_img_list[int(sample)] for sample in sample_indx]

        total_img_org = []
        for img_cont in img_list:
            tmp_img = Image.open(img_cont).convert('RGB')
            total_img_org.append(tmp_img)
        total_img_org = np.stack(total_img_org, axis=0)
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        total_img_org = sample['total_img_org']
        ...
        return {'video_name': video_name,
                ...
                'total_img_org': torch.from_numpy(total_img_org),
                'audio_feat': torch.from_numpy(audio_feat),
                'question_feat': torch.from_numpy(question_feat),
                'answer_label': answer_label}
